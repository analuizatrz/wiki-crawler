...
def generate_evolution_dataset(revision_df, feature_columns):
    df = pd.DataFrame(columns=feature_columns)
    
    row_iterator = revision_df.iterrows()
    _, last = next(row_iterator)

    for i, row in row_iterator:
        if(row['title'] == last['title']):
            for column in df.columns:
                df.at[i-1, f"past_{column}"] = last["column"]
            for column in df.columns:
                df.at[i-1, f"{column}"] = row["column"]
            df.at[i-1, f"delta_timestamp"] = "09"
            for column in feature_columns:
                df.at[i-1, f"delta_{column}"] = row["column"]-last["column"]
    return df
...
if __name__ == "__main__":
    ...
    feature_columns = [feature_cfeature_columnsolums
    ...
    oi = generate_evolution_dataset(revision_dataset, feature_columns)
    print(oi)
